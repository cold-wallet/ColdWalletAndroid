...
open class ChildItem(val title: String, val isCrypto: Boolean) : Item() {
    ...
    override fun bind(viewHolder: GroupieViewHolder, position: Int) {
        ...
        if(isCrypto){
            viewHolder.root.setBackgroundColor(getColor(viewHolder.root.context,R.color.colorBgCrypto))
        }else{
            viewHolder.root.setBackgroundColor(getColor(viewHolder.root.context,R.color.colorBgFiat))
        }
